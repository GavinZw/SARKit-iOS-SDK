...
static bool	_isUnityRunning	= false;
static UnityAppController *unityAppController = [UnityAppController sharedInstance];
...
+ (UnityAppController *)sharedInstance{
  if (!unityAppController) {
    unityAppController = [[UnityAppController alloc] init];
  }
...
- (void)startUnityApp{
  if (!_isUnityRunning) {
    ...
    [self applicationDidBecomeActive:self.application];
  }
}

- (void)stopUnityApp{
  if (_isUnityRunning) {
    [self applicationWillResignActive:self.application];
